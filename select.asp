...
<body>
	...
	================================================== -->
		<%
			...
			sql_string = Request.Form("select")

			if INSTR(LCASE(sql_string), "update") > 0 OR _
				 INSTR(LCASE(sql_string), "insert") > 0 OR _
         INSTR(LCASE(sql_string), "drop")   > 0 OR _
				 INSTR(LCASE(sql_string), "create") > 0 _
			then
		
				response.write "<span class='text text-warning'><h1 class='page-header'>Go away Huzefa</h3></span>"	
